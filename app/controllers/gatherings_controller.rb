class GatheringsController < ApplicationController
  ...
  def create
    ...
    p Gathering.first
    p '################## query ################'
    gathering = Gathering.find_by(leader_id: params[:params][:leader_id], user_id: params[:user_id])
    if gathering
      gathering.query = params[:params][:query]
      if gathering.query == ""
        gathering.destroy
      else
        gathering.save
      ...
      gathering = Gathering.create(leader_id: leader.id, user_id: user.id, query: params[:params][:query])
      ################################
